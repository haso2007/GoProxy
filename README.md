...
- 免费池 / 订阅池分离展示，实时状态监控
- 代理集中管理：管理员可多选代理并批量删除；删除后进入手动筛选模式，后台补池、优化替换、订阅刷新不会新增代理，实际代理服务只使用剩余列表
- 手动恢复全量池：点击「抓取代理」会清空临时删除记录并恢复后台补池 / 订阅刷新
...
管理员在代理列表中单个或批量删除代理后，系统会进入临时手动筛选模式：

- 被删除的代理地址会被临时屏蔽，浏览器刷新、刷新延迟、后台补池都不会把它们重新加入
- 后台自动抓取补池、优化器替换、订阅刷新、禁用订阅代理探测恢复会暂停新增代理
- HTTP / SOCKS5 代理服务只会从当前剩余的数据库代理中选择，适合临时只保留指定地区或指定出口
- 点击 WebUI 的「抓取代理」会清空临时删除记录，退出手动筛选模式，并重新抓取可用代理
- 该模式为运行期内存状态，应用重启后会恢复正常自动管理
